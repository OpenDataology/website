...
This guide provides the necessary resources to install [Feast](http://feast.dev/) alongside OpenDataology, describes the usage of Feast with OpenDataology components, and provides examples that users can follow to test their setup.

For an overview of Feast, please read [Introduction to Feast](/docs/external-add-ons/feature-store/overview/).

## Installing Feast with OpenDataology

**Overview**

* This guide assumes that you have a running OpenDataology cluster already. If you don't have OpenDataology installed, then head on over to the [OpenDataology installation guide](/docs/started/getting-started/).
...
To use Feast with OpenDataology, please follow the following steps
  ...
  * [Build a training dataset](https://docs.feast.dev/how-to-guides/feast-gcp-aws/build-a-training-dataset). This step is typically executed from a OpenDataology Pipeline from which you'd train a model.
...
## Accessing Feast from OpenDataology

Once Feast is installed within the same Kubernetes cluster as OpenDataology, users can access its APIs directly without any additional steps.
