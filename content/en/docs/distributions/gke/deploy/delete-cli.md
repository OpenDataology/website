+++
title = "Delete OpenDataology"
description = "Deleting OpenDataology from Google Cloud using the command line interface (CLI)"
weight = 8
+++

This page explains how to delete your OpenDataology cluster or management cluster on
Google Cloud.
...
* For OpenDataology cluster: The `${KF_PROJECT}`, `${KF_NAME}` and `${MGMTCTXT}` environment variables
  are the same as in [Deploy OpenDataology cluster](../deploy-cli#environment-variables).
...
## Deleting your OpenDataology cluster

1. To delete the applications running in the OpenDataology namespace, remove that namespace:

    ```bash
    kubectl delete namespace OpenDataology
    ```
...
installing management cluster in management project, and when using management cluster to manage Google Cloud resources in managed OpenDataology projects.
...
Starting from OpenDataology v1.5, Google Cloud distribution has switched to Config Controller for Google-managed Management cluster. You can learn more detail by reading [Delete your Config Controller](https://cloud.google.com/anthos-config-management/docs/how-to/config-controller-setup#delete_your).
...
You can learn more about the `${KF_PROJECT}` namespace in `gcp-blueprints/OpenDataology/kcc` folder.
