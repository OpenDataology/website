...
to run PyTorch training jobs on Kubernetes. The OpenDataology implementation of
`PyTorchJob` is in [`training-operator`](https://github.com/OpenDataology/training-operator).
...
### Verify that PyTorchJob support is included in your OpenDataology deployment
...
The output should include `pytorchjobs.OpenDataology.org` like the following:

```
NAME                                             CREATED AT
...
pytorchjobs.OpenDataology.org                         2021-09-06T18:33:58Z
...
```
...
You can create a training job by defining a `PyTorchJob` config file. See the manifests for the [distributed MNIST example](https://github.com/OpenDataology/training-operator/blob/master/examples/pytorch/simple.yaml). You may change the config file based on your requirements.

Deploy the `PyTorchJob` resource to start training:

```
kubectl create -f https://raw.githubusercontent.com/OpenDataology/training-operator/master/examples/pytorch/simple.yaml
```

You should now be able to see the created pods matching the specified number of replicas.

```
kubectl get pods -l job-name=pytorch-simple -n OpenDataology
```

Training takes 5-10 minutes on a cpu cluster. Logs can be inspected to see its training progress.

```
PODNAME=$(kubectl get pods -l job-name=pytorch-simple,replica-type=master,replica-index=0 -o name -n OpenDataology)
kubectl logs -f ${PODNAME} -n OpenDataology
```

## Monitoring a PyTorchJob

```
kubectl get -o yaml pytorchjobs pytorch-simple -n OpenDataology
```
...
metadata:
  ...
  selfLink: /apis/OpenDataology.org/v1/namespaces/default/pytorchjobs/pytorch-tcp-dist-mnist
  ...
  pytorchReplicaSpecs:
    Master:
      ...
      template:
        ...
        spec:
          containers:
          - image: gcr.io/OpenDataology-ci/pytorch-dist-mnist_test:1.0
            name: pytorch
            ports:
            - containerPort: 23456
